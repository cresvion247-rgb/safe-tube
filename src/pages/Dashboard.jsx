...
export default function Dashboard() {
  const { t } = useI18n();
  const [authState, setAuthState] = useState("checking"); // checking | signedIn | signedOut
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 border-b border-border bg-background pt-safe">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 p-4 sm:p-6">
          <Link
            to="/"
            className="flex h-12 shrink-0 items-center gap-2 rounded-full border border-border bg-card px-4 font-medium hover:bg-accent"
          >
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button
              type="button"
              onClick={signOut}
              className="flex h-12 items-center gap-2 rounded-full border border-border bg-card px-4 text-sm font-medium text-muted-foreground hover:bg-accent hover:text-foreground"
            >
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          {TABS.map(({ id, labelKey, icon: Icon }) => (
            <button
              ...
              className={`flex h-12 items-center gap-2 rounded-full border-2 px-5 text-sm font-bold ${
          ...
          <div className="space-y-6">
            {editing === null && (
              <button
                ...
                className="flex h-14 items-center gap-2 rounded-2xl bg-primary px-6 font-heading text-lg font-bold text-primary-foreground shadow-lg"
              >
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {t("dash.profileMeta", {
                          age: t(`ageGroup.${profile.ageGroup}`),
                          minutes: profile.dailyTimeLimitMinutes,
                          tokens: profile.educationalTokens,
                        })}{" "}
                        · {profile.targetLanguages.join(", ").toUpperCase()}
                      </p>
                    </div>
                    <div className="flex shrink-0 gap-2">
                      <button
                        ...
                        className="grid h-12 w-12 place-items-center rounded-xl text-muted-foreground hover:bg-accent hover:text-foreground"
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          if (window.confirm(t("dash.removeConfirm", { name: profile.childName }))) {
                        ...
                        aria-label={t("dash.deleteProfile", { name: profile.childName })}
                        className="grid h-12 w-12 place-items-center rounded-xl text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
                      >
